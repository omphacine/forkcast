...
export default async function RecipePage({
  ...
  // Not my own recipe — see if someone shared it with me (either meal-plan or
  // recipes access grants viewing an individual recipe: meal-plan access
  // needs it for clicking through from a planned meal; recipes access needs
  // it for browsing at /recipes/shared).
  const access = await getSharedAccess();
  const sharedRecipe =
    access && (access.canViewMealPlan || access.canViewRecipes)
      ? await getRecipe(recipeId, access.ownerUserId)
      : undefined;
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-8 px-6 py-10">
        ...
        <div>
          <div className="flex items-center justify-between gap-4">
            <h1 className="font-heading text-4xl font-semibold">{sharedRecipe.name}</h1>
            <form action={importRecipe.bind(null, recipeId)}>
              <button
                type="submit"
                className="shrink-0 rounded-full bg-primary px-4 py-2 text-base font-medium text-white hover:opacity-90"
              >
          ...
          {sharedRecipe.rating && (
            <p
              ...
              aria-label={`${sharedRecipe.rating} out of 5 stars`}
            >
              {"★".repeat(sharedRecipe.rating)}
              <span className="text-foreground/30">{"★".repeat(5 - sharedRecipe.rating)}</span>
            </p>
          )}
        </div>

        {sharedRecipe.photoDataUrl && (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={sharedRecipe.photoDataUrl}
            alt=""
            className="w-full rounded-lg object-cover"
          />
        )}

        {sharedRecipe.sourceName && (
          <div>
            <h2 className="font-heading text-2xl font-semibold">Source</h2>
            {sharedSourceLink ? (
              <SourceLink sourceName={sharedRecipe.sourceName} />
            ) : (
              <p className="mt-1 text-lg text-foreground/80">
                {sharedRecipe.sourceName}
                {sharedRecipe.sourcePage && `, p. ${sharedRecipe.sourcePage}`}
              </p>
            )}
          </div>
        )}

        {sharedRecipe.instructions && (
          <div>
            <h2 className="font-heading text-2xl font-semibold">Instructions</h2>
            <p className="mt-2 whitespace-pre-wrap text-lg text-foreground/80">
              {sharedRecipe.instructions}
        ...
        <div>
          ...
          {sharedRecipe.ingredients.length === 0 ? (
            ...
            <ul className="mt-2 flex flex-col gap-1">
              {sharedRecipe.ingredients.map((ingredient) => (
        ...
        {sharedRecipe.plannedDates.length > 0 && (
          <div>
            <h2 className="font-heading text-2xl font-semibold">Planned</h2>
            <ul className="mt-4 flex flex-col gap-2">
              {sharedRecipe.plannedDates.map((planned) => (
                <li
                  key={planned.entryId}
  ...
  // Not signed in, not my recipe, and not shared with me — fall back to a
  // read-only view instead of a hard 404, unless the recipe genuinely
  // doesn't exist (e.g. a stranger opening a link from a shared calendar
  // event, or a signed-out visitor).
