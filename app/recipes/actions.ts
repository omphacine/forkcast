...
// Persists a full new ordering after a drag-to-reorder — orderedIds is every
// ingredient id for this recipe, in its new order.
export async function reorderRecipeIngredients(recipeId: number, orderedIds: number[]) {
  const userId = await getUserId();
  const [owned] = await sql`SELECT id FROM recipes WHERE id = ${recipeId} AND user_id = ${userId}`;
  if (!owned) return;
  ...
  for (let i = 0; i < orderedIds.length; i++) {
    await sql`
      UPDATE recipe_ingredients SET position = ${i}
      WHERE id = ${orderedIds[i]} AND recipe_id = ${recipeId}
    `;
  }
